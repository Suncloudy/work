...
module.exports.invoiceAddressAdd=async (ctx,next)=>{
    ctx.checkBody('receiver').notEmpty("接收人不能为空");
    ...
    let invoiceAddressDa=ctx.dao.main.invoice.InvoiceAddress;
    await invoiceAddressDa.create(
        {
    ...
    ctx.body={code:Code.OK.code,msg:'地址添加成功'};
    return;
}

module.exports.invoiceAddressList=async (ctx,next)=>{
    ctx.checkParams('userId').notEmpty("用户id不能为空");
    ...
    let paginate=await invoiceAddressDa.paginate(option);
    let items = [];
...
module.exports.invoiceAddressEdit=async (ctx,next)=>{
...
module.exports.invoiceAddressDefault=async (ctx,next)=>{
    ctx.checkParams('addressId').notEmpty("地址id不能为空");
    ctx.checkBody('default').notEmpty("地址状态不能为空");
    let body = ctx.request.body
    let params=ctx.params;
...
module.exports.invoiceAddressDelete=async (ctx,next)=>{
    ctx.checkParams('addressId').notEmpty("地址id不能为空");
    let params = ctx.params;
    let errors='';
    if(ctx.errors){
    ...
    let invoiceAddressDa=ctx.dao.main.invoice.InvoiceAddress;
    await invoiceAddressDa.destroy({where:{id:params.addressId}});
    ctx.body={code:Code.OK.code,msg:'成功'};
    return;
}